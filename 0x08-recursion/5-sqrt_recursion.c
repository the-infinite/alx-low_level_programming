/**
 * _sqrt_recursion_wrapper - a wrapper that does the recursion part of finding
 * the square root of a number.
 *
 * @n : the number to wortk with.
 * @min: minimum possible value to guess
 * @max: maximum possible value to guess.
 *
...
int _sqrt_recursion_wrapper(int n, int min, int max)
{
	...
	if (guess_squared == n)
	{
		/*if squared guess == n return guess*/
		return (guess);
	}
	...
	{
		/*overshoot g if g squared is less than n*/
		return (_sqrt_recursion_wrapper(n, guess + 1, max));
	}

	else
	{
		/*undershoot g if sg is more than the n*/
